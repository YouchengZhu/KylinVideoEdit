...
int AudioEdit::audioSplit(QString inputPath, QString outputPath1, QString outputPath2, const double time)
{
    int ret1 = audioIntercept(inputPath, outputPath1,0,time);
    int ret2 = audioIntercept(inputPath, outputPath2,time, 0);
...
int AudioEdit::audioIntercept(const QString filepath1, const QString filepath2, const double start_time, const double end_time)
{
    ...
    cmd = "ffmpeg -i " + str1 + " -ss " + doubleToString(start_time) + " -t " + doubleToString(end_time-start_time) + " -acodec copy "  +  str2 + " -y";
